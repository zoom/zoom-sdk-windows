...
{
	//to do
	//need a new class to handle this
}
	...
	if (!m_bUIIsReady)
	{
		...
		{			
			if (m_pCustomizeUIVideoMgr)
			{
				...
				//wilmer check it later, doesn't it a right place to set m_pCustomizeUIVideoMgr to be NULL?
				//the right way should be set it to be NULL 
				m_pCustomizeUIVideoMgr = NULL;
			}
			...
			if (m_pCustomizeShareMgr)
			{
				...
				//wilmer check it later, doesn't it a right place to set m_pCustomizeShareMgr to be NULL?
				m_pCustomizeShareMgr = NULL;
			}
		}
	}
...
{
	//to do
	////Class CCustomizeUIShareMgr handles this
}
