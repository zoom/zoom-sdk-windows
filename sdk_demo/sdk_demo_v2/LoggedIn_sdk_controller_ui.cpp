...
void CSDKLoggedInUIMgr::ShowUI(LoggedIn_MeetingUI_type nType_)
{
	...
	switch(nType_)
	{
		/* Following code is used for rest api join meeting. If no need. should be removed before release -- wilmer comments
...
void CSDKLoggedInUIMgr::DoStartMeetingBtnClick(bool bHasVideo)
{
	...
	if(m_loggedInUIWorkFlow.StartMeeting(startParam))
	{
		m_bConnectToMeeting = true;
		m_bInMeeting = false;
	}
