...
public:
  ...
  virtual void onUserJoin(ZOOM_SDK_NAMESPACE::IList<unsigned int> *lstUserID,
                          const wchar_t *strUserList = NULL) = 0;
  ...
  virtual void onUserAudioStatusChange(
      ...
      const wchar_t *strAudioStatusList = NULL) = 0;
  virtual void
  onRecordingStatus(ZOOM_SDK_NAMESPACE::RecordingStatus status) = 0;
  virtual void onChatMsgNotifcation(ZOOM_SDK_NAMESPACE::IChatMsgInfo *chatMsg,
                                    const wchar_t *ccc) = 0;
  virtual void
  onUserVideoStatusChange(unsigned int userId,
                          ZOOM_SDK_NAMESPACE::VideoStatus status) = 0;
  ...
  virtual void onLowOrRaiseHandStatusChanged(bool bLow,
                                             unsigned int userid) = 0;
  virtual void onCustomizedLocalRecordingSourceNotification(
      ZOOM_SDK_NAMESPACE::ICustomizedLocalRecordingLayoutHelper
          *layout_helper) = 0;
};
...
public:
  ...
  virtual void onMeetingSecureKeyNotification(
  ...
  virtual void
  onCloudRecordingStatus(ZOOM_SDK_NAMESPACE::RecordingStatus status) {}
  ...
  virtual void onUserVideoStatusChange(unsigned int userId,
  ...
  virtual void onHostRequestStartAudio(
      ZOOM_SDK_NAMESPACE::IRequestStartAudioHandler *handler_) {}

  // IMeetingChatCtrlEvent
  virtual void onChatMsgNotifcation(ZOOM_SDK_NAMESPACE::IChatMsgInfo *chatMsg,
                                    const wchar_t *ccc);
  virtual void
  onChatStautsChangedNotification(ZOOM_SDK_NAMESPACE::ChatStatus *status_) {}
  ...
  virtual void
  onShareContentNotification(ZOOM_SDK_NAMESPACE::ShareInfo &shareInfo);
...
private:
  ...
  ZOOM_SDK_NAMESPACE::IMeetingVideoController *m_pVideoCtrl;
	ZOOM_RAWDATA_NAMESPACE::IAudioRawDataChannel *m_pAudioRawDataChannel{nullptr};

	RawAudioDataHandler *m_pRawAudioDataHandler{nullptr};
	RawVideoDataHandler *m_pRawVideoDataHandler{nullptr};
};
