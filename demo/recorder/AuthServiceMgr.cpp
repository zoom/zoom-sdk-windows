...
bool CAuthServiceMgr::Init() {
  ...
  if (ZOOM_SDK_NAMESPACE::CreateAuthService(&m_pAuthService) !=
      ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS)
    return false;
...
bool CAuthServiceMgr::UnInit() {
  ...
  if (ZOOM_SDK_NAMESPACE::DestroyAuthService(m_pAuthService) !=
      ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS)
    return false;
...
void CAuthServiceMgr::onAuthenticationReturn(
    ZOOM_SDK_NAMESPACE::AuthResult ret) {
  if (m_pSink) {
    m_pSink->onAuthenticationReturn(ret);
  }
}

void CAuthServiceMgr::onLoginRet(
