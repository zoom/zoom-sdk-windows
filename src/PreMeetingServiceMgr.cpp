...
bool CPreMeetingServiceMgr::Init() {
  if (ZOOM_SDK_NAMESPACE::CreatePreMeetingService(&m_pPreMeetingService) !=
  ...
  if (m_pPreMeetingService->SetEvent(this) !=
      ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS)
    return false;
...
void CPreMeetingServiceMgr::onListMeeting(
    ZOOM_SDK_NAMESPACE::PremeetingAPIResult result,
    ZOOM_SDK_NAMESPACE::IList<UINT64> *lstMeetingList) {
  if (ZOOM_SDK_NAMESPACE::PREMETAPIRET_SUCCESS == result) {
    OutputMeetingListInfo(lstMeetingList);
  }
}

void CPreMeetingServiceMgr::onScheduleOrEditMeeting(
    ZOOM_SDK_NAMESPACE::PremeetingAPIResult result, UINT64 meetingUniqueID) {
  if (ZOOM_SDK_NAMESPACE::PREMETAPIRET_SUCCESS == result) {
    OutputMeetingListInfo(meetingUniqueID);
  }
}

void CPreMeetingServiceMgr::onDeleteMeeting(
    ZOOM_SDK_NAMESPACE::PremeetingAPIResult result) {
  if (m_pPreMeetingService)
    m_pPreMeetingService->ListMeeting();
}

void CPreMeetingServiceMgr::OutputMeetingListInfo(
    ZOOM_SDK_NAMESPACE::IList<UINT64> *lstMeetingList) {
  if (lstMeetingList) {
...
void AppendToString(const wchar_t *str_name, UINT64 number_,
...
void AppendToString(const wchar_t *str_name, const wchar_t *str_,
                    std::wstring &out_put_str) {
  if (str_name) {
    out_put_str += str_name;
    out_put_str += L"-->\n";
...
#define APPEND_TO_WITH_PARAM2(key_, param_, param_2)                           \
  AppendToString(__ST(key_), meeting_item_info->key_(param_, param_2),         \
                 out_put_str);
void CPreMeetingServiceMgr::OutputMeetingListInfo(UINT64 meetingUniqueID) {
  if (m_pPreMeetingService) {
    ZOOM_SDK_NAMESPACE::IMeetingItemInfo *meeting_item_info =
        m_pPreMeetingService->GetMeeingItem(meetingUniqueID);
...
void CPreMeetingServiceMgr::OutputMeetingListInfo(
  ...
  if (meeting_item_info) {
    ...
    if (!meeting_item_info->IsPMI() && !meeting_item_info->IsWebinar()) {
      ...
      AppendToString(L"IsScheduleWithPmiEnabled-canchang", can_change,
                     out_put_str);
    }
  }
  ...
  ZOOM_SDK_NAMESPACE::SCHEDULEAUDIOTYPE type(
      ZOOM_SDK_NAMESPACE::SCHEDULEAUDIOTYPE_none);
  ...
  AppendToString(L"IsAutoRecordEnabled-support_rec_type", support_rec_type,
                 out_put_str);
  ...
  ZOOM_SDK_NAMESPACE::IQueryMeetingItemTelAudioInfoHelper *meeting_item_tel =
      meeting_item_info->GetQueryTelAudioInfoHelper();
  if (meeting_item_tel) {
    ZOOM_SDK_NAMESPACE::IList<const wchar_t *> *lst_support_tel =
        meeting_item_tel->GetAvailableDialinCountryCode();
    if (lst_support_tel) {
    ...
    ZOOM_SDK_NAMESPACE::IList<const wchar_t *> *lst_select_tel =
        meeting_item_tel->GetSelectedDialinCountryCode();
    if (lst_select_tel) {
    ...
    AppendToString(L"is include tollfree:",
                   meeting_item_tel->IsIncludeTollFree(), out_put_str);
  }
...
void CPreMeetingServiceMgr::TestScheduleMeeting() {
  if (m_pPreMeetingService) {
    ZOOM_SDK_NAMESPACE::IScheduleMeetingItem *schedule_meeting_item =
        m_pPreMeetingService->CreateScheduleMeetingItem();
    if (schedule_meeting_item) {
      OutputDebugString(L"Schedule meeting default vaule begin:\n");
      OutputMeetingListInfo(
          dynamic_cast<ZOOM_SDK_NAMESPACE::IMeetingItemInfo *>(
              schedule_meeting_item));
      ...
      bool can_change(false);
      if (!schedule_meeting_item->IsMeetingPasswordEnabled(can_change) &&
          can_change) {
        schedule_meeting_item->EnableMeetingPassword(true);
        schedule_meeting_item->SetMeetingPassword(L"Test@123");
      }

      if (!schedule_meeting_item->IsJoinBeforeHostEnabled(can_change) &&
          can_change) {
        schedule_meeting_item->EnableJoinBeforeHost(true);
      }

      if (!schedule_meeting_item->IsMuteUponEntryEnabled(can_change) &&
          can_change) {
        schedule_meeting_item->EnableMuteUponEntry(true);
      }

      if (!schedule_meeting_item->IsScheduleWithPmiEnabled(can_change) &&
          can_change) {
        schedule_meeting_item->EnableScheduleWithPmi(true);
      }

      if (!schedule_meeting_item->IsHostInChinaEnabled(can_change) &&
          can_change) {
        schedule_meeting_item->EnableHostInChina(true);
      }

      int support_rec_type = 0;
      if (!schedule_meeting_item->IsAutoRecordEnabled(can_change,
                                                      support_rec_type) &&
          can_change) {
